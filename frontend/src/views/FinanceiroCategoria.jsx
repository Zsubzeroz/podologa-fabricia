...
import { List, Plus, Search, Trash2, Edit, CheckSquare } from 'lucide-react';

export default function FinanceiroCategoria() {
  const [categories, setCategories] = useState(() => {
    const saved = window.localStorage.getItem('finance_categories');
    return saved ? JSON.parse(saved) : [
      { id: 1, nome: 'Despesas Financeiras', status: 'ATIVO' },
      { id: 2, nome: 'Taxas Bancárias', status: 'ATIVO' },
      { id: 3, nome: 'Despesas Fixas', status: 'ATIVO' },
      { id: 4, nome: 'Água', status: 'ATIVO' },
      { id: 5, nome: 'Aluguel', status: 'ATIVO' },
      { id: 6, nome: 'Celular / Internet', status: 'ATIVO' },
    ];
  });
  ...
  useEffect(() => {
    window.localStorage.setItem('finance_categories', JSON.stringify(categories));
  }, [categories]);
  ...
  const handleOpenEditModal = (cat) => {
    setFormData({ nome: cat.nome, status: cat.status });
    setEditId(cat.id);
    setIsEditing(true);
    setShowModal(true);
  };
  ...
  const handleSaveCategory = (e) => {
    ...
    if (isEditing) {
      const updated = categories.map(c => c.id === editId ? { ...c, nome: formData.nome, status: formData.status } : c);
          ...
          <table className="sa-table" style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #f3f4f6' }}>
            <thead>
              <tr style={{ background: '#f9fafb', borderBottom: '2px solid #e5e7eb' }}>
                <th style={{ padding: '14px', textAlign: 'left', color: '#374151', fontSize: '0.9rem', fontWeight: '700' }}>NOME DA CATEGORIA</th>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan="3" style={{ textAlign: 'center', padding: '30px', color: '#6b7280' }}>Nenhuma categoria financeira encontrada.</td>
                </tr>
              ) : (
                filtered.map((cat) => (
                  <tr key={cat.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '14px', color: '#111827', fontWeight: 'bold', fontSize: '0.95rem' }}>{cat.nome}</td>
                    <td style={{ padding: '14px', textAlign: 'center' }}>
                      <span style={{ 
                        background: cat.status === 'ATIVO' ? '#ecfdf5' : '#fef2f2', 
                        color: cat.status === 'ATIVO' ? '#047857' : '#b91c1c', 
                        padding: '4px 12px', 
                        borderRadius: '6px', 
                        fontWeight: 'bold',
                        fontSize: '0.85rem'
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <label style={{ fontSize: '13px', color: '#374151', fontWeight: '600' }}>Nome da Categoria</label>
              <input 
                type="text" 
                required 
                value={formData.nome}
