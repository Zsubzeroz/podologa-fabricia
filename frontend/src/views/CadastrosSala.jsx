...
import { Home, Plus, Search, Trash2, Edit } from 'lucide-react';

export default function CadastrosSala() {
  const [salas, setSalas] = useState(() => {
    const saved = window.localStorage.getItem('salas_list');
    return saved ? JSON.parse(saved) : [
      { id: 1, nome: 'Sala de Podologia', ordem: 1, multiplosAgendamentos: 'NÃO', status: 'ATIVO' }
    ];
  });
  ...
  const [formData, setFormData] = useState({
    nome: '',
    ordem: '',
    multiplosAgendamentos: 'NÃO',
    status: 'ATIVO'
  });

  useEffect(() => {
    window.localStorage.setItem('salas_list', JSON.stringify(salas));
  }, [salas]);
  ...
  const handleOpenEditModal = (sala) => {
    setFormData({
      nome: sala.nome,
      ordem: sala.ordem,
      multiplosAgendamentos: sala.multiplosAgendamentos,
      status: sala.status
    });
    setEditId(sala.id);
    setIsEditing(true);
  ...
  const handleSaveSala = (e) => {
    e.preventDefault();
    if (!formData.nome.trim()) return;

    if (isEditing) {
      const updated = salas.map(s => s.id === editId ? { ...formData, id: editId, ordem: parseInt(formData.ordem) || 1 } : s);
      setSalas(updated);
    } else {
      const newS = {
        id: Date.now(),
          ...
          <table className="sa-table" style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #f3f4f6' }}>
            <thead>
              <tr style={{ background: '#f9fafb', borderBottom: '2px solid #e5e7eb' }}>
                <th style={{ padding: '14px', textAlign: 'left', color: '#374151', fontSize: '0.9rem', fontWeight: '700' }}>NOME DA SALA</th>
                <th style={{ padding: '14px', textAlign: 'center', color: '#374151', fontSize: '0.9rem', fontWeight: '700' }}>ORDEM</th>
                <th style={{ padding: '14px', textAlign: 'center', color: '#374151', fontSize: '0.9rem', fontWeight: '700' }}>MÚLTIPLOS AGENDAMENTOS</th>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan="5" style={{ textAlign: 'center', padding: '30px', color: '#6b7280' }}>Nenhuma sala cadastrada.</td>
                </tr>
              ) : (
                filtered.map((sala) => (
                  <tr key={sala.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '14px', color: '#111827', fontWeight: 'bold', fontSize: '0.95rem' }}>{sala.nome}</td>
                    <td style={{ padding: '14px', color: '#4b5563', textAlign: 'center' }}>{sala.ordem}</td>
                    <td style={{ padding: '14px', color: '#4b5563', textAlign: 'center' }}>{sala.multiplosAgendamentos}</td>
                    <td style={{ padding: '14px', textAlign: 'center' }}>
                      <span style={{ 
                        background: sala.status === 'ATIVO' ? '#ecfdf5' : '#fef2f2', 
                        color: sala.status === 'ATIVO' ? '#047857' : '#b91c1c', 
                        padding: '4px 12px', 
                        borderRadius: '6px', 
                        fontWeight: 'bold',
                        fontSize: '0.85rem'
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <label style={{ fontSize: '13px', color: '#374151', fontWeight: '600' }}>Nome da Sala</label>
              <input 
                type="text" 
                name="nome"
                required 
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <label style={{ fontSize: '13px', color: '#374151', fontWeight: '600' }}>Ordem de Exibição</label>
                <input 
                  type="number" 
                  name="ordem"
                  required 
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <label style={{ fontSize: '13px', color: '#374151', fontWeight: '600' }}>Agend. Múltiplos?</label>
                <select 
                  name="multiplosAgendamentos"
                  value={formData.multiplosAgendamentos}
                  onChange={handleChange}
                  style={{ padding: '10px', border: '1px solid #d1d5db', borderRadius: '6px', outline: 'none' }}
                >
