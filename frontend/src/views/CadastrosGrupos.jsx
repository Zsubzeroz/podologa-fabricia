export default function CadastrosGrupos() {
  const grupos = [
    { grupo: 'Podólogia', subgrupo: 'Clinico' },
    { grupo: 'podólogia', subgrupo: 'Terapia' },
    { grupo: 'Produto', subgrupo: 'Novo' },
  ];

  return (
    <div>
      <div className="sa-page-header">
        <h2 className="sa-page-title" style={{fontWeight: 'bold', color: '#337ab7'}}>GRUPOS DE PRODUTOS E SERVIÇOS ❓</h2>
      </div>
      
      <div style={{padding: '20px'}}>
        <div style={{display: 'flex', gap: '15px', marginBottom: '20px', alignItems: 'center'}}>
          <button className="btn btn-primary" style={{backgroundColor: '#3fa9f5'}}>+ NOVO GRUPO</button>
          ...
          <div style={{display: 'flex'}}>
             ...
             <div style={{background: '#f5f5f5', border: '1px solid #ccc', borderLeft: 'none', padding: '5px 10px', fontSize: '13px', borderRadius: '0 3px 3px 0'}}>3 registro(s)</div>
          </div>
        </div>
        ...
        <table className="sa-table" style={{border: '1px solid #eee', background: 'white'}}>
          <thead>
            <tr style={{borderBottom: '2px solid #ddd'}}>
              <th style={{color: '#555', width: '40%'}}>GRUPO</th>
              <th style={{color: '#555', width: '40%'}}>SUBGRUPO</th>
              <th style={{color: '#555'}}>STATUS</th>
              <th style={{color: '#555', textAlign: 'center'}}>AÇÃO</th>
            </tr>
          </thead>
          <tbody>
            {grupos.map((g, i) => (
              <tr key={i}>
                <td style={{color: '#337ab7'}}>{g.grupo}</td>
                <td style={{color: '#337ab7'}}>{g.subgrupo}</td>
                <td>
