...
export default function AuditoriaAgenda() {
  ...
  useEffect(() => {
    const data = AuditManager.getAll().filter(log => log.acao.includes('Agendamento') || log.acao.includes('Agenda'));
    setLogs(data);
    setFiltered(data);
  }, []);
              ...
              {filtered.map((log) => (
                <tr key={log.id}>
                  ...
                  <td style={{ padding: '15px', borderBottom: '1px solid #f3f4f6' }}>
                    <span style={{ 
                      backgroundColor: log.acao.includes('Cancelado') || log.acao.includes('Excluir') ? '#fef2f2' : '#ecfdf5', 
                      color: log.acao.includes('Cancelado') || log.acao.includes('Excluir') ? '#dc2626' : '#059669', 
                      padding: '4px 10px', 
                      borderRadius: '9999px', 
                      fontSize: '0.7rem', 
                      fontWeight: '700' 
